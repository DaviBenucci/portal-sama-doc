...
| Autenticação | Parcial | Existe autenticação PHP por sessão no legado. A base NestJS agora existe em `/api-v2/auth/*` com JWT de acesso, refresh cookie HttpOnly e CSRF double-submit assinado para `login`, `refresh` e `logout`; ainda não foi ligada ao frontend nem validada contra MySQL real. |
