...
| CSRF da autenticação NestJS precisa ser validado em fluxo real | Requisições mutáveis com cookie HttpOnly | `portal-sama-api/src/modules/auth/*`, `portal-sama-web/scripts/portal-auth-smoke.mjs` | CSRF double-submit assinado foi implementado em Auth; `smoke:auth` foi criado para validar `csrf -> login -> me -> refresh -> me -> logout` com usuario real sem expor segredos. Falta executar em HTTPS/MySQL real. | Parcial |
