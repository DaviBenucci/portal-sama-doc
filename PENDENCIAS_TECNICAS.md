...
- Implementado rate limit configuravel com `ACESSORIAS_RATE_LIMIT_PER_MINUTE=95`, para manter margem abaixo do limite de 100 requisicoes por minuto.
- Implementado retry controlado para `429`, respeitando `Retry-After` quando o Acessorias informar.
- Sync de entregas passou a registrar e reutilizar `incremental_since` do ultimo run `SUCCESS`.
- O Web passou a usar timeout de 10 minutos nas chamadas Acessorias e a mostrar erro por acao no painel DEV.
- `Importar tudo` passou a chamar cadastros com `entity=all`, reduzindo repeticao quando responsaveis forem extraidos de empresas.
- Criada a rota React `/departamentos/vencimentos`, com filtros por departamento, mes, origem, status e busca, consolidando vencimentos de calendario e Acessorias.
- Validacoes locais passaram: 4 suites focadas com 20 testes na API, TypeScript do Web, build/lint API e build/lint Web.
- Permanece pendente validar no EasyPanel com dados reais, totais reais de clientes/entregas, tempo real de importacao e a tela da Central antes de qualquer homologacao.

## Atualizacao 2026-06-01 vencimentos Acessorias no workspace

- Implementada a conexao local entre entregas/vencimentos do Acessorias e o carrossel de vencimentos do workspace departamental.
- `DepartmentsService` agora inclui entregas de `acessorias_deliveries` com `dueAt` no mes selecionado e status pendente/atrasado no mesmo pacote de vencimentos das regras de calendario.
- Quando existe mapeamento `CONFIRMED` para uma coluna operacional valida, o vencimento Acessorias tambem aparece na celula e participa do bloqueio por vencimento; sem mapeamento confirmado, aparece apenas no carrossel.
- `/departamentos/modelo` passou a sinalizar no carrossel os itens com origem `Acessorias`.
- Validacoes locais passaram: `departments.service.spec.ts` com 5 testes, build/lint API e build/lint Web.
- Permanece pendente validar com dados reais no EasyPanel, mapeamentos reais de obrigacoes/departamentos, Central de Vencimentos dedicada, scheduler e notificacoes.
...
- Permanece pendente validar no EasyPanel com dados reais, usuario autorizado, CSRF real e auditoria persistida; Central de Vencimentos dedicada, scheduler e notificacoes continuam pendentes.
...
- Permanece pendente validar dados reais no EasyPanel, revisar mapeamentos de obrigacoes por departamento e conectar a Central de Vencimentos/notificacoes.
...
- O smoke valida anonimos esperando 401 em `/auth/me`, `/users` e `/documents`, e aceita matriz JSON com perfis, `usernameEnv`/`passwordEnv`, checks por rota/metodo e status esperado.
- O script mantem cookies em memoria, mascara usuarios e nao imprime senha, access token, refresh token ou CSRF token.
- Passaram `node --check`, help do comando, smoke `--soft --json` contra porta fechada, smoke positivo contra servidor fake local com ADMIN/CLIENT/anonimo, lint e build do Web.
- Permanece pendente: rodar `npm.cmd run smoke:permissions` contra `https://portal.samacontabil.com.br/api-v2` com usuarios reais de homologacao por perfil, registrar evidencia sanitizada e cruzar com matriz 401/403/200.

## Atualizacao 2026-05-25 17:54 -03:00

- `portal-sama-web` agora possui `npm.cmd run test:e2e:real`, com `playwright.real.config.ts`, para executar Playwright contra o dominio publico sem subir servidor local.
- A spec `tests/e2e/real-auth.spec.ts` cobre login pela UI React, chegada na Home, ausencia de chaves sensiveis no storage do navegador, refresh cookie fora de `document.cookie`, politica `HttpOnly`/`SameSite`/`Secure` e logout.
- A execucao real fica opt-in por `PORTAL_REAL_E2E=1` e credenciais em variaveis de ambiente; trace, screenshot e video ficam desligados nessa config.
- Passaram `npm.cmd run test:e2e:real` sem variaveis com 1 skipped, `npm.cmd run lint`, `npm.cmd run build`, `npm.cmd run test:e2e` com 9 testes e 1 skipped, e `git diff --check` no Web.
- Permanece pendente: rodar a suite contra `https://portal.samacontabil.com.br` com usuario real de homologacao, registrar evidencia sem segredos e repetir por perfis criticos.

## Atualizacao 2026-05-25 17:37 -03:00

- `portal-sama-web` agora possui `npm.cmd run smoke:auth`, com `scripts/portal-auth-smoke.mjs`, para validar autenticacao real em homologacao sem expor senha/tokens.
- O smoke cobre `csrf -> login -> me -> refresh -> me -> logout`, mantendo cookies em memoria e validando politica do refresh cookie (`HttpOnly`, `SameSite` e `Secure` quando HTTPS).
- Passaram `node --check`, help do comando, smoke positivo contra servidor fake local, smoke `--soft` sem credenciais, lint e build do Web.
- Permanece pendente: rodar `npm.cmd run smoke:auth` contra `https://portal.samacontabil.com.br/api-v2` com usuario real de homologacao, registrar evidencia sem segredos e repetir por perfis criticos.

## Atualizacao 2026-05-25 17:19 -03:00

- O fluxo publico de documentos de onboarding foi alinhado ao contrato documentado: `GET /api-v2/public/onboarding/documents/:token` e `POST /api-v2/public/onboarding/documents/:token/upload` agora existem na API v2.
- Os endpoints genericos `GET /api-v2/documents/public-checklist?token=...` e `POST /api-v2/documents/public-upload?token=...` foram mantidos para compatibilidade operacional.
- `portal-sama-web/src/services/documents.service.ts` passou a usar os aliases publicos de onboarding a partir da rota React `/onboarding/publico/documentos/:token`.
- Smoke Playwright local da tela publica foi atualizado para o novo alias e passou com API mockada.
- Permanecem pendentes: validar tokens reais/legados, upload real, MySQL/storage/ClamAV em homologacao, Playwright de upload publico e evidencia HTTPS no EasyPanel.
...
- A rota React publica `/onboarding/publico/documentos/:token` ganhou `PublicDocumentsPage.tsx`; em 2026-05-25 passou a consumir os aliases especificos `GET /api-v2/public/onboarding/documents/:token` e `POST /api-v2/public/onboarding/documents/:token/upload`, mantendo compatibilidade com `POST /api-v2/documents/public-upload?token=...`.
...
| Criar frontend React/Vite | Frontend | Alta | `portal-sama-web/`, `docs/MATRIZ_MIGRACAO_HTML_PARA_REACT.md`, `docs/paginas/` | Parcial | Fundacao, certificados, clientes/cadastro/painel/visao geral/departamental, colaboradores, DEV colaboradores, carteira do gestor, transferencias, documentos internos, documentos do painel, documentos publicos de onboarding, solicitacoes, auditoria, notificacoes, legalizacao, onboarding/processos, DEV/admin, propostas, contratos/assinatura e TI operacional iniciais criados; Playwright inicial cobre login/documentos publicos/auditoria, `test:e2e:real` cobre login/Home/logout real de forma opt-in e `smoke:permissions` cobre matriz 401/403/200 por perfil. Proxima acao e executar em homologacao/HTTPS com usuarios reais, ampliar Playwright, telas gerenciais/contabeis e homologar fluxos reais. |
...
| CSRF da autenticacao NestJS precisa ser validado em fluxo real | Requisicoes mutaveis com cookie HttpOnly | `portal-sama-api/src/modules/auth/*`, `portal-sama-web/scripts/portal-auth-smoke.mjs`, `portal-sama-web/scripts/portal-permissions-smoke.mjs`, `portal-sama-web/tests/e2e/real-auth.spec.ts` | CSRF double-submit assinado foi implementado em Auth; `smoke:auth` foi criado para validar `csrf -> login -> me -> refresh -> me -> logout`, `smoke:permissions` para matriz 401/403/200 por perfil e `test:e2e:real` para validar login/Home/logout via navegador sem trace/video/screenshot. Falta executar em HTTPS/MySQL real com usuarios reais. | Parcial |
...
|---|---|---|---|---|
| Migrar autenticação PHP para NestJS | AuthModule | `portal-sama-api/src/modules/auth/*`, `portal-sama-web/src/services/auth.service.ts`, `portal-sama-web/scripts/portal-auth-smoke.mjs`, `api/auth.php`, `api/storage.php`, `Login.js`, `auth.js` | Parcial | Base `/api-v2/auth/login`, `/refresh`, `/logout`, `/me`, `/csrf` e `/forgot-password` criada, frontend React integrado e smoke operacional de autenticacao real disponibilizado; falta rodar contra MySQL/HTTPS real por perfil e registrar evidencias. |
...
|---|---|---|---|---|
| Migrar login | `/login` | `portal-sama-web/src/pages/auth/LoginPage.tsx`, `index.html`, `Login.js`, `docs/paginas/index.md`, `portal-sama-web/tests/e2e/real-auth.spec.ts` | Parcial | Tela React criada usando API v2; `test:e2e:real` cobre login, Home, storage sem chaves sensiveis, politica do refresh cookie e logout. Falta executar com MySQL/homologacao, usuario real, HTTPS e fluxo de erro/refresh em navegador. |
...
| Migrar documentos publicos de onboarding | `/onboarding/publico/documentos/:token` | `Onboarding/documentos-cliente.html`, `Onboarding/documentos-cliente.js`, `portal-sama-api/src/modules/documents/*`, `portal-sama-web/src/pages/documents/PublicDocumentsPage.tsx` | Parcial | Tela React publica criada com checklist por `GET /api-v2/public/onboarding/documents/:token` e envio multipart por `POST /api-v2/public/onboarding/documents/:token/upload`; endpoints genericos de documentos seguem compativeis. Smoke Playwright inicial cobre a rota com API mockada e foi atualizado para o alias. Falta validacao com API v2/MySQL/storage/ClamAV reais, tokens legados, HTTPS/homologacao e Playwright de upload. |
