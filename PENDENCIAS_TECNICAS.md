...
| Criar frontend React/Vite | Frontend | Alta | `portal-sama-web/`, `docs/MATRIZ_MIGRACAO_HTML_PARA_REACT.md`, `docs/paginas/` | Parcial | Fundacao, certificados, clientes/cadastro/painel/visao geral/departamental, colaboradores, DEV colaboradores, carteira do gestor, transferencias, documentos internos, documentos do painel, documentos publicos de onboarding, solicitacoes, auditoria, notificacoes, legalizacao, onboarding/processos, DEV/admin, propostas, contratos/assinatura e TI operacional iniciais criados; Playwright inicial cobre login/documentos publicos/auditoria e `test:e2e:real` cobre login/Home/logout real de forma opt-in; proxima acao e executar em homologacao/HTTPS com usuario real, ampliar Playwright, telas gerenciais/contabeis e homologar fluxos reais. |
...
| CSRF da autenticacao NestJS precisa ser validado em fluxo real | Requisicoes mutaveis com cookie HttpOnly | `portal-sama-api/src/modules/auth/*`, `portal-sama-web/scripts/portal-auth-smoke.mjs`, `portal-sama-web/tests/e2e/real-auth.spec.ts` | CSRF double-submit assinado foi implementado em Auth; `smoke:auth` foi criado para validar `csrf -> login -> me -> refresh -> me -> logout` e `test:e2e:real` foi criado para validar login/Home/logout via navegador sem trace/video/screenshot. Falta executar em HTTPS/MySQL real com usuario real. | Parcial |
