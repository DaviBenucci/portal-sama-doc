...
Atualizacao em 2026-06-02 17:05: `UsersService.update()` passou a exigir papel `MASTER` para qualquer atualizacao de senha em usuarios existentes. Quando a troca e permitida, o fluxo continua aplicando bcrypt, CSRF via controller, permissao `users.update` e auditoria `users.update` com `passwordChanged=true`, sem registrar senha ou hash em metadata. Validar no EasyPanel com usuario MASTER real e conferir auditoria persistida.
