...
- Continuidade de 2026-06-25: adotado tratamento por polling para o `acessorias-controlled-sync`. O backend marca os runs do operacional com trigger `manual_operational_sync`; a tela `/dev/fase-9-smoke` mostra `sync-runs` recentes; e o runner `smoke:phase9` passa a consultar `GET /integrations/acessorias/sync-runs` apos timeout local/HTTP 504, aprovando a acao somente se catalogo e entregas terminarem `SUCCESS`.
...
- Implementacao local posterior: `smoke:phase9` ganhou `--acessorias-poll-timeout`/`PORTAL_PHASE9_ACESSORIAS_POLL_TIMEOUT_MS` para transformar esse caso em validacao por `sync-runs`, sem imprimir corpo bruto, tokens ou cookies. Validacoes focadas iniciais passaram com `node --check scripts/portal-phase9-smoke.mjs`, `npm.cmd test -- --runInBand` no web e `npm.cmd test -- --runInBand src/modules/integrations/acessorias/acessorias-operational-sync.service.spec.ts` na API.
...
- Apos a correcao, duas rodadas paralelas de `homologation:real` criaram evidencias distintas sem `EEXIST`: publica `ok=true` em `.ai-tests/homologation-real-phase9/homologation-real-20260622T195954391Z.json` e integrada `ok=false` apenas por credenciais ausentes em `.ai-tests/homologation-real-phase9/homologation-real-20260622T195954392Z.json`.
- `git diff --check` - OK.
...
- Apos o runner automatizado, nova rodada publica com `--skip-phase9` retornou `ok=true` e gerou evidencia ignorada pelo git em `.ai-tests/homologation-real-phase9/homologation-real-20260622T195241Z.json`.
- Apos o deploy informado em 2026-06-22, a rota `/dev/fase-9-smoke` foi novamente validada em producao via `smoke:phase9`, retornando HTTP 200 e `bodyLooksLikeHtml=true`.
...
- Validar o novo polling de `acessorias-controlled-sync` em ambiente alvo apos deploy, confirmando que catalogo e entregas aparecem como `SUCCESS` em `sync-runs` quando a resposta direta exceder o timeout publico.
...
Comando recomendado para a homologacao final com e2e real:
...
- O proximo trabalho deve configurar credenciais de teste e executar `smoke:phase9` read-only e depois com `PORTAL_PHASE9_APPLY_ACTIONS=1`, preservando/copy-out da evidencia operacional da Fase 8. A ultima validacao publica pos-deploy ja confirmou a rota da Fase 9 em producao.
