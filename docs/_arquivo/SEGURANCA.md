...
Atualizacao em 2026-05-26 16:50: `portal-sama-api` agora possui `npm run ops:restore:drill` para preflight e restore em alvo isolado. O comando valida backup por `ops:backup:verify`, bloqueia alvo igual a `DATABASE_URL`/`STORAGE_PRIVATE_PATH` e exige `RESTORE_DRILL_TARGET_IS_ISOLATED` para aplicar restauracao. Falta executar com backup real no EasyPanel e registrar evidencia sanitizada.
...
Atualizacao em 2026-06-03 10:15: `UsersService.update()` passou a exigir papel `DEV` para qualquer atualizacao de senha em usuarios existentes. Quando a troca e permitida, o fluxo continua aplicando bcrypt, CSRF via controller, permissao `users.update` e auditoria `users.update` com `passwordChanged=true`, sem registrar senha ou hash em metadata. `MASTER` nao deve ser tratado como permissao total equivalente na nova stack. Validar no EasyPanel com usuario DEV real e conferir auditoria persistida.
