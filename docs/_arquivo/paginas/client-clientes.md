...
- **Arquivo HTML:** `Client/clientes.html`
- **Módulo:** Clientes
- **Arquivos CSS relacionados:** Nenhum CSS relacionado identificado diretamente.
- **Arquivos JavaScript relacionados:** Nenhum JavaScript relacionado identificado diretamente.
- **APIs/endpoints relacionados:** Nenhum endpoint identificado diretamente.
- **Perfil de usuário provável:** Usuário autenticado que usa rota legada de clientes
- **Nível de criticidade:** Baixo
- **Prioridade de refatoração:** Baixa

**Status backend 2026-05-13 10:50:** `ClientsModule` iniciou `/api-v2/clients`; esta pagina segue como rota de compatibilidade/redirecionamento, mas o destino futuro deve consumir a listagem tipada da API v2.

---

## 2. Objetivo da página

Redirecionar para a visão consolidada de clientes.
...
- **Quem utiliza:** Usuário autenticado que usa rota legada de clientes.
- **Etapa do fluxo:** Clientes dentro do Portal Sama.
- **Dados/documentos manipulados:** não manipula dados diretamente.
...
- Não há fluxo de autenticação explícito identificado no HTML. Ponto a validar no código/backend.
...
> Nenhum endpoint identificado diretamente nesta página. Recomenda-se validar chamadas indiretas nos arquivos JavaScript relacionados.
...
- Redirecionamento automático para rota principal atual.
- Não deve haver regra de negócio sensível no HTML de redirecionamento.

### Regras que devem existir obrigatoriamente no backend

- Rotas finais devem validar sessão e permissão.
...
Não há `auth.js` relacionado diretamente a esta página. Ponto a validar no código/backend.
...
Dados sensíveis envolvidos: não manipula dados diretamente. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
- Não foram encontrados problemas estruturais específicos além dos riscos gerais do projeto: JS vanilla acoplado, ausência de tipagem e ausência de documentação formal de contratos de API.
...
- React + TypeScript + Vite para componentização gradual sem exigir reescrita imediata de todo o portal.
...
A página `Client/clientes.html` atua no módulo **Clientes** e manipula **não manipula dados diretamente**. Os principais riscos são: autorização por perfil/departamento e exposição de dados. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
...
- **Arquivo HTML atual:** `Client/clientes.html`
- **Rota React sugerida:** `/clientes`
- **Componente React sugerido:** `ClientsPage.tsx`
...
- A rota React `/clientes` passou a renderizar `portal-sama-web/src/pages/clients/ClientsPage.tsx`.
- A tela consome `/api-v2/clients` por `clients.service.ts`, com tipos em `types/clients.ts` e validacao Zod em `schemas/client.schema.ts`.
- A primeira fatia cobre filtros por busca/status/CNPJ/cidade/grupo, paginacao, cadastro, edicao e arquivamento logico por permissao.
- As permissoes `clients.create`, `clients.update` e `clients.delete` ocultam acoes no frontend, mas a autorizacao definitiva segue no `ClientsModule` com JWT, CSRF, permissoes e auditoria.
- Validacao local: `npm.cmd run lint` passou; `npm.cmd run build` falhou inicialmente por tipo do schema Zod/React Hook Form, foi corrigido e passou.
- Pendencias: validar com MySQL real, backfill legado, vinculos cliente-usuario/gestor, Playwright e fluxo com usuarios/permissoes reais.
