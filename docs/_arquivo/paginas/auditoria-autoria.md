...
- **Arquivo HTML:** `Auditoria/autoria.html`
- **Módulo:** Auditoria
- **Arquivos CSS relacionados:** `global.css`, `DEV/dev.css`, `Auditoria/autoria.css`, `https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css`
- **Arquivos JavaScript relacionados:** `auth.js`, `global.js`, `Auditoria/autoria.js`
- **APIs/endpoints relacionados:** `/api/notifications.php actions: ack, alert_ack, clear_unread, list`, `/api/storage.php actions: audit_list, audit_push, audit_trash_move, auth_logout, auth_session_status, backup_now, backup_status, get, restore_select, set, user_presence_ping`
...
Consultar logs de auditoria e acionar backup/restauração do sistema.

- **Problema resolvido:** centraliza o fluxo de auditoria para reduzir consulta manual, duplicação operacional e perda de rastreabilidade.
- **Quem utiliza:** Somente master.
- **Etapa do fluxo:** Auditoria dentro do Portal Sama.
- **Dados/documentos manipulados:** logs de auditoria, usuário, ação, detalhes, backups e restauração.
...
Conteúdo principal identificado: Autoria Auditoria simplificada + Backup/Restauração (somente MASTER) Usuário ▾ Departamento do usuário Logout Atualizar 90 dias Mostrar tudo Backup Restaurar Lo.

Títulos/seções visíveis: `Autoria`, `Logs`, `Restaurar backup`.

Tabelas identificadas: `auditTable` (`Data/Hora`, `Usuário`, `Ação`, `Detalhes`).

Campos relevantes: `auditSearch` (text).

Ações/botões identificados: `Início`, `Departamento ▾`, `Modelo padrão Departamento`, `Clientes`, `Área Dev`, `Usuário ▾`, `Departamento do usuário`, `Logout`, `Atualizar`, `90 dias`, `Mostrar tudo`, `Backup`, `Restaurar`, `×`, `... mais 1 item(ns)`.
...
- Endpoints envolvidos: `/api/notifications.php`, `/api/storage.php`. As actions observadas são: `ack`, `alert_ack`, `clear_unread`, `list`, `audit_list`, `audit_push`, `audit_trash_move`, `auth_logout`, `auth_session_status`, `backup_now`, `backup_status`, `get`, `restore_select`, `set`, `user_presence_ping`.
...
Campos/dados de entrada identificados no HTML: `auditSearch` (text)
...
- **Action observada:** `audit_list`
  - **Método provável:** POST
  - **Finalidade:** Listar eventos de auditoria.
  - **Dados enviados:** Filtros de busca/período.
  - **Dados retornados:** Lista de eventos.
  - **Validações esperadas:** Somente master.
  - **Riscos de segurança:** Exposição de dados sensíveis em logs.
  - **Melhorias recomendadas:** Mascarar campos sensíveis e aplicar retenção.
...
- **Action observada:** `audit_trash_move`
  - **Método provável:** POST
  - **Finalidade:** Mover eventos para descarte/lixeira lógica.
  - **Dados enviados:** IDs/critério de eventos.
  - **Dados retornados:** Confirmação.
  - **Validações esperadas:** Somente master e CSRF.
  - **Riscos de segurança:** Remoção de evidências de auditoria.
  - **Melhorias recomendadas:** Auditoria imutável e retenção legal.
...
- **Action observada:** `backup_now`
  - **Método provável:** POST
  - **Finalidade:** Criar backup manual.
  - **Dados enviados:** Sessão master e CSRF.
  - **Dados retornados:** Resultado do backup.
  - **Validações esperadas:** Somente master e CSRF.
  - **Riscos de segurança:** Abuso para exfiltração ou DoS.
  - **Melhorias recomendadas:** Guardar em storage privado com criptografia e trilha de auditoria.
- **Action observada:** `backup_status`
  - **Método provável:** GET
  - **Finalidade:** Consultar estado de backup.
  - **Dados enviados:** Sessão master.
  - **Dados retornados:** Status de backup.
  - **Validações esperadas:** Somente master.
  - **Riscos de segurança:** Exposição de caminhos/estado operacional.
  - **Melhorias recomendadas:** Não retornar caminhos internos sensíveis.
...
- **Action observada:** `restore_select`
  - **Método provável:** POST
  - **Finalidade:** Restaurar backup selecionado.
  - **Dados enviados:** Tipo de backup.
  - **Dados retornados:** Resultado de restauração.
  - **Validações esperadas:** Somente master e CSRF.
  - **Riscos de segurança:** Perda de dados se restaurado indevidamente.
  - **Melhorias recomendadas:** Confirmação dupla, janela de manutenção e backup prévio automático.
...
- Master filtra logs, consulta eventos e aciona backup/restauração.
- Tela possui janela/modal de restauração.

### Regras que devem existir obrigatoriamente no backend

- Somente master.
- Auditoria append-only, sem edição destrutiva.
- Backup criptografado e restauração com confirmação dupla.
- Logs sem dados sensíveis em claro.
...
Dados sensíveis envolvidos: logs de auditoria, usuário, ação, detalhes, backups e restauração. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
A página `Auditoria/autoria.html` atua no módulo **Auditoria** e manipula **logs de auditoria, usuário, ação, detalhes, backups e restauração**. Os principais riscos são: administração privilegiada. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
...
- **Arquivo HTML atual:** `Auditoria/autoria.html`
- **Rota React sugerida:** `/auditoria`
- **Componente React sugerido:** `AuditPage.tsx`
