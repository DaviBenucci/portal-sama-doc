...
- Criacao/retorno de transferencias continuam em `/manager/transferencias`; a escrita normalizada de transferencias em lote foi implementada localmente no `TransfersModule`, e o backfill seguro de `clients.metadata` foi instrumentalizado por `ops:client-assignments:backfill`, mas ambos ainda dependem de validacao real/aplicacao controlada. Adicionar empresas sem responsavel e historico/calendario completos seguem dependentes de contratos futuros e validacao.
