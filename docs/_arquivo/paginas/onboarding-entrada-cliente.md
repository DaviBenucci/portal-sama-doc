...
- **Arquivo HTML:** `Onboarding/entrada-cliente.html`
- **Módulo:** Onboarding
- **Arquivos CSS relacionados:** `global.css`, `https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css`
- **Arquivos JavaScript relacionados:** `auth.js`, `global.js`, `Onboarding/entrada-cliente.js`
- **APIs/endpoints relacionados:** `/api/notifications.php actions: ack, alert_ack, clear_unread, create, list`, `/api/onboarding.php actions: create, delete, get`, `/api/storage.php actions: audit_push, auth_logout, auth_session_status, delete, get, set, user_presence_ping`
- **Perfil de usuário provável:** Usuário interno autorizado para criar processos
...
Criar e acompanhar processos de entrada de cliente por tipo de serviço e regime/lucro.
...
- **Quem utiliza:** Usuário interno autorizado para criar processos.
- **Etapa do fluxo:** Onboarding dentro do Portal Sama.
- **Dados/documentos manipulados:** tipo de serviço, regime, status das etapas e processo de onboarding.
...
Conteúdo principal identificado: Entrada de Cliente Esteira de onboarding Usuário ▾ Departamento Logout Tipo de serviço (Todos) Comércio Prestador de Serviço Lucro (Todos) Simples Presumido Rea; Tipo de serviço (Todos) Comércio Prestador de Serviço Lucro (Todos) Simples Presumido Real Iniciar processo 1. Envio de Proposta 2. Envio de Contrato 3. Documen.

Títulos/seções visíveis: `Entrada de Cliente`.

Campos relevantes: `f_tipo` (select), `f_lucro` (select).

Ações/botões identificados: `Início`, `Departamento ▾`, `Modelo padrão`, `Clientes`, `Área Dev`, `Usuário ▾`, `Departamento`, `Logout`, `Iniciar processo`.
...
- Endpoints envolvidos: `/api/notifications.php`, `/api/onboarding.php`, `/api/storage.php`. As actions observadas são: `ack`, `alert_ack`, `clear_unread`, `create`, `list`, `audit_push`, `auth_logout`, `auth_session_status`, `delete`, `get`, `set`, `user_presence_ping`, `create`, `delete`, `get`.
...
Campos/dados de entrada identificados no HTML: `f_tipo` (select), `f_lucro` (select)
...
Dados sensíveis envolvidos: tipo de serviço, regime, status das etapas e processo de onboarding. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
A página `Onboarding/entrada-cliente.html` atua no módulo **Onboarding** e manipula **tipo de serviço, regime, status das etapas e processo de onboarding**. Os principais riscos são: autorização por perfil/departamento e exposição de dados. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
...
- **Arquivo HTML atual:** `Onboarding/entrada-cliente.html`
- **Rota React sugerida:** `/onboarding/entrada-cliente`
- **Componente React sugerido:** `ClientEntryPage.tsx`
...
## 16. Atualizacao de frontend React - 2026-05-18 14:52

- A rota `/onboarding/entrada-cliente` passou a renderizar `OnboardingClientEntryPage.tsx`.
- A tela consome `POST /api-v2/onboarding/processes` para iniciar processos e `GET /api-v2/onboarding/processes` para consultar a esteira recente com filtros por busca, servico, regime e status.
- Quando a sessao possui `clients.read`, a tela consulta `GET /api-v2/clients` e permite preencher empresa, CNPJ e contato a partir de cliente ja cadastrado.
- A navegacao foi ligada no menu, na Home e na tela `/onboarding/processos`.
- Permanecem pendentes validacao com API v2/MySQL/usuarios reais, maquina de estados completa, links publicos especificos, backfill legado, Playwright e homologacao HTTPS.
