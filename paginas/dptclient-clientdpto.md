...
- **Perfil de usuário provável:** Colaborador ou gestor com departamento permitido
...
Exibir clientes associados ao departamento do usuário e permitir atribuição ou transferência de responsáveis.

- **Problema resolvido:** centraliza o fluxo de departamento / clientes para reduzir consulta manual, duplicação operacional e perda de rastreabilidade.
- **Quem utiliza:** Colaborador ou gestor com departamento permitido.
- **Etapa do fluxo:** Departamento / Clientes dentro do Portal Sama.
- **Dados/documentos manipulados:** clientes, CNPJ, status, responsável do departamento e vínculos de carteira.
...
Conteúdo principal identificado: Clientes do Departamento — Nome do colaborador ▾ Departamento do usuário Logout Clientes — Somente sem responsável Atribuir/transferir para... Selecionar sem re; Clientes — Somente sem responsável Atribuir/transferir para... Selecionar sem responsável Aplicar aos selecionados Cliente CNPJ Telefone Endereço Grupo Responsá; Cliente CNPJ Telefone Endereço Grupo Responsável (no depto) Tempo no status Status Nenhum cliente encontrado..

Títulos/seções visíveis: `Clientes do Departamento`, `Clientes`.

Tabelas identificadas: `tabelaDeptClientes` (`Cliente`, `CNPJ`, `Telefone`, `Endereço`, `Grupo`, `Responsável (no depto)`, `Tempo no status`, `Status`).

Campos relevantes: `onlyUnassigned` (checkbox), `assignTarget` (select), `checkAll` (checkbox).

Ações/botões identificados: `Início`, `Departamento ▾`, `Modelo padrão Departamento`, `Clientes`, `Área Dev`, `Nome do colaborador ▾`, `Departamento do usuário`, `Logout`, `Selecionar sem responsável`, `Aplicar aos selecionados`.
...
Dados sensíveis envolvidos: clientes, CNPJ, status, responsável do departamento e vínculos de carteira. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
- [x] Permissões validadas no backend para leitura da primeira tela React (`clients.read`).
- [x] Inputs validados/controlados no frontend React da primeira tela.
- [ ] Inputs validados no backend.
- [x] Dados sensíveis não expostos no HTML inicial da primeira tela React.
- [x] Dados sensíveis não persistidos no JavaScript/storage da primeira tela React.
- [x] Tokens não armazenados de forma insegura na sessao React.
...
- [ ] Ações críticas registradas em auditoria; atribuicao/transferencia ainda nao migradas.
...
A página `DptClient/clientdpto.html` atua no módulo **Departamento / Clientes** e manipula **clientes, CNPJ, status, responsável do departamento e vínculos de carteira**. Os principais riscos são: autorização por perfil/departamento e exposição de dados. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
