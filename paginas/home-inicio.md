...
- **Arquivo HTML:** `HOME/Inicio.html`
- **Módulo:** Home / Dashboard
- **Arquivos CSS relacionados:** `https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css`, `global.css`, `HOME/inicio.css`
- **Arquivos JavaScript relacionados:** `auth.js`, `DEV/dev-data.js`, `https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js`, `global.js`, `HOME/inicio.js`
...
- **Perfil de usuário provável:** Usuário autenticado, com menus ajustados por perfil e departamento
...
Apresentar indicadores gerais de clientes, colaboradores, status e vencimentos para acompanhamento interno.

- **Problema resolvido:** centraliza o fluxo de home / dashboard para reduzir consulta manual, duplicação operacional e perda de rastreabilidade.
- **Quem utiliza:** Usuário autenticado, com menus ajustados por perfil e departamento.
- **Etapa do fluxo:** Home / Dashboard dentro do Portal Sama.
- **Dados/documentos manipulados:** clientes, colaboradores, status, calendário de vencimentos e notificações.
...
Conteúdo principal identificado: Seja Bem-Vindo Resumo das informações totais Nome do colaborador ▾ Departamento do usuário Logout Mês Ano Hoje Exportar Total de Clientes — Base total Clientes; Mês Ano Hoje Exportar Total de Clientes — Base total Clientes Novos — Últimos 30 dias Ativos — Clientes ativos Inativos — Clientes inativos Bloqueados — Acesso; Status dos Clientes Alternar detalhes; Novos Clientes (12 meses) —.

Títulos/seções visíveis: `Seja Bem-Vindo`, `Status dos Clientes`, `Novos Clientes (12 meses)`, `Calendário de Vencimentos`, `Colaboradores por Departamento`, `Últimos 10 Clientes Cadastrados`, `Total de Clientes`, `Clientes Novos`, `Ativos`, `Inativos`, `Bloqueados`, `Dia --`, `... mais 1 item(ns)`.

Tabelas identificadas: `lastClientsTable` (`Nome`, `CNPJ`, `Status`, `Regime`, `Cadastro`).

Campos relevantes: `dashMonth` (select), `dashYear` (select), `homeCalDept` (select).

Ações/botões identificados: `Início`, `Departamento ▾`, `Modelo padrão Departamento`, `Clientes`, `Área Dev`, `Nome do colaborador ▾`, `Departamento do usuário`, `Logout`, `Hoje`, `Exportar`, `Alternar detalhes`, `◀`, `▶`, `Fechar`, `... mais 1 item(ns)`.
...
Campos/dados de entrada identificados no HTML: `dashMonth` (select), `dashYear` (select), `homeCalDept` (select)
...
- Regras aparentes são carregadas por JavaScript relacionado à página. Ponto a validar no código/backend.

### Regras que devem existir obrigatoriamente no backend

- Autenticação, autorização, validação server-side e auditoria devem existir para qualquer ação crítica.
...
Foram identificadas 45 ocorrência(s) de `innerHTML` nos scripts vinculados. Evitar inserir dados de API com `innerHTML`; preferir `textContent` ou sanitização contextual.
...
Dados sensíveis envolvidos: clientes, colaboradores, status, calendário de vencimentos e notificações. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
- Arquivos JavaScript extensos/acoplados à tela: `DEV/dev-data.js` (2582 linhas), `global.js` (2433 linhas), `HOME/inicio.js` (1212 linhas).
...
A página `HOME/Inicio.html` atua no módulo **Home / Dashboard** e manipula **clientes, colaboradores, status, calendário de vencimentos e notificações**. Os principais riscos são: autorização por perfil/departamento e exposição de dados. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
...
- A rota React `/home` passou a usar o novo padrao visual do `portal-sama-web` com `PageShell`, `KpiCard`, `AppPanel` e `ShortcutCard`.
- Os atalhos continuam filtrados por permissoes da sessao e renderizados como links reais.
- Nao houve novo contrato backend para esta tela; os dados seguem limitados ao snapshot de sessao atual.
- Pendencias: conectar indicadores operacionais reais quando houver contratos especificos e criar QA visual/Playwright para Home e sidebar recolhida.

### Atualizacao 2026-05-22 09:28 -03:00

- A pendencia de Playwright para Home/sidebar foi reduzida em `portal-sama-web/tests/e2e/smoke.spec.ts`.
- A suite agora valida `/home` autenticado, badge de sessao, KPI de permissoes e atalhos por permissao no conteudo principal.
- A sidebar agora tem teste versionado para largura fechada `96px`, hover `304px`, fechamento apos clique de mouse e abertura por foco de teclado.
- Validacao final: `npm.cmd run lint`, `npm.cmd run build`, `npm.cmd run test:e2e` com 5 testes e `git diff --check` passaram.
- Pendencias restantes: QA visual manual em navegador real, mobile/homologacao e indicadores operacionais reais.

### Atualizacao 2026-05-22 09:39 -03:00

- A cobertura Playwright da Home/sidebar foi ampliada para mobile em viewport `390x844`.
- O novo teste valida que a sidebar nao fica compacta no mobile, mostra labels, usa grid de navegacao em uma coluna e nao cria overflow horizontal.
- Validacao: `npm.cmd run lint`, `npm.cmd run build` e `npm.cmd run test:e2e` passaram; a suite agora possui 6 testes Chromium.
- Pendencias restantes: QA visual manual em navegador real, homologacao com usuario/permissoes reais e indicadores operacionais reais.
...
- **Arquivo HTML atual:** `HOME/Inicio.html`
- **Rota React sugerida:** `/home`
- **Componente React sugerido:** `HomePage.tsx`
