...
- **Arquivo HTML:** `Client/visao-geral-colaboradores.html`
...
- **Arquivos JavaScript relacionados:** `auth.js`, `DEV/dev-data.js`, `global.js`, `Client/visao-geral-colaboradores.js`
...
- **Perfil de usuário provável:** Usuário autenticado, gestor ou master para visão consolidada
...
Listar colaboradores, departamentos, cargos, clientes e status operacional.
...
- **Quem utiliza:** Usuário autenticado, gestor ou master para visão consolidada.
- **Etapa do fluxo:** Clientes / Colaboradores dentro do Portal Sama.
- **Dados/documentos manipulados:** dados funcionais de colaboradores e vínculos com clientes.
...
Conteúdo principal identificado: Visão Geral Colaboradores Nome do colaborador ▾ Departamento do usuário Logout Colaboradores Lista consolidada de colaboradores e suas informações gerais. Todos; Colaboradores Lista consolidada de colaboradores e suas informações gerais. Todos os departamentos Todos os status Ativo Inativo Bloqueado Nome Departamento Car; Nome Departamento Cargo Clientes Ramal E-mail corporativo Status Nenhum colaborador encontrado..

Títulos/seções visíveis: `Visão Geral`, `Colaboradores`.

Tabelas identificadas: `tabelaVisaoColabs` (`Nome`, `Departamento`, `Cargo`, `Clientes`, `Ramal`, `E-mail corporativo`, `Status`).

Campos relevantes: `filtroDept` (select), `filtroStatus` (select).
...
Campos/dados de entrada identificados no HTML: `filtroDept` (select), `filtroStatus` (select)
...
Dados sensíveis envolvidos: dados funcionais de colaboradores e vínculos com clientes. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
A página `Client/visao-geral-colaboradores.html` atua no módulo **Clientes / Colaboradores** e manipula **dados funcionais de colaboradores e vínculos com clientes**. Os principais riscos são: autorização por perfil/departamento e exposição de dados. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
...
- **Arquivo HTML atual:** `Client/visao-geral-colaboradores.html`
- **Rota React sugerida:** `/colaboradores/visao-geral`
- **Componente React sugerido:** `CollaboratorsOverviewPage.tsx`
...
- A tela passa a ter contrato alvo inicial em `GET /api-v2/collaborators`, com filtros por busca, status, departamento e role.
- A API exige JWT e `collaborators.read`; perfis nao privilegiados ficam limitados ao proprio departamento.
- Ainda faltam backfill de `sama_colaboradores`, carteira/vinculos cliente-colaborador e migracao React.

## 17. Atualizacao de frontend React - 2026-05-18 09:17

- A rota `/colaboradores/visao-geral` passou a renderizar `CollaboratorsOverviewPage.tsx`.
- A tela consome `GET/POST/PATCH/DELETE /api-v2/collaborators` por meio de `collaborators.service.ts`, com schema Zod e tipos TypeScript dedicados.
- A primeira experiencia cobre filtros por busca, status, departamento, perfil e arquivados, estatisticas da pagina, detalhe seguro, cadastro, edicao, troca de status/perfis e arquivamento conforme permissoes.
- Ainda faltam validacao com MySQL/usuarios reais, backfill de `sama_colaboradores`, vinculos cliente-colaborador/gestor, carteira/transferencias e Playwright.
