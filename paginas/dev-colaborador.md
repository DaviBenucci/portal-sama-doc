...
- **Arquivo HTML:** `DEV/dev-colaborador.html`
- **Módulo:** DEV / Colaboradores
- **Arquivos CSS relacionados:** `DEV/dev.css`, `global.css`, `https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css`
- **Arquivos JavaScript relacionados:** `auth.js`, `DEV/dev-data.js`, `DEV/dev-colaborador.js`, `global.js`
- **APIs/endpoints relacionados:** `/api/manager_workspace.php`, `/api/notifications.php actions: ack, alert_ack, clear_unread, list`, `/api/storage.php actions: audit_push, auth_logout, auth_session_status, get, set, user_presence_ping`
...
Editar colaborador, status, senha, permissões, departamentos e carteira de clientes.

- **Problema resolvido:** centraliza o fluxo de dev / colaboradores para reduzir consulta manual, duplicação operacional e perda de rastreabilidade.
- **Quem utiliza:** Master ou administrador técnico.
- **Etapa do fluxo:** DEV / Colaboradores dentro do Portal Sama.
- **Dados/documentos manipulados:** dados de colaborador, credenciais, role, permissões, clientes vinculados e status.
...
Conteúdo principal identificado: Portal Sama Contabilidade Painel de Visualização (Colaborador) Nome do colaborador ▾ Departamento do usuário Configurações do perfil Logout ← Voltar para Área D; ← Voltar para Área DEV Colaborador • ATIVO Ativo Inativo Bloqueado Excluir Geral Clientes Permissões Informações do colaborador Nome Departamento principal Carg; Informações do colaborador Nome Departamento principal Cargo Status Resumo de clientes Total de clientes sob responsabilidade (no depto principal): Um mesmo cli; Clientes do colaborador Lista do departamento principal do colaborador. Transferir para... Transferir selecionados Transferir todos Adicionar empresas sem respo; Permissões de acesso O perfil define o nível (Colaborador / Gestor / Master) e os departamentos definem o que ele pode visualizar/gerir. Perfil Colaborador Gest.

Títulos/seções visíveis: `Portal Sama Contabilidade`, `Colaborador`, `Informações do colaborador`, `Resumo de clientes`, `Editar dados do colaborador`.

Tabelas identificadas: `tabelaClientesColab` (`Cliente`, `CNPJ`, `Departamento`).

Campos relevantes: `editNomeCompleto` (text), `editUsername` (text), `editDepartamentoPrincipal` (select), `editRamal` (text), `editEmailCorporativo` (email), `editSenha` (password), `editSenhaConfirmacao` (password), `transferTarget` (select), `addSemRespSelect` (select), `checkAllClientes` (checkbox), `perfilRole` (select), `perfilDepts` (select).

Ações/botões identificados: `Início`, `Departamento ▾`, `Modelo padrão Departamento`, `Clientes`, `Todos os Clientes`, `Área Dev`, `Nome do colaborador ▾`, `Departamento do usuário`, `Configurações do perfil`, `Logout`, `← Voltar para Área DEV`, `ATIVO`, `Ativo`, `Inativo`, `... mais 9 item(ns)`.
...
- Endpoints envolvidos: `/api/manager_workspace.php`, `/api/notifications.php`, `/api/storage.php`. As actions observadas são: `ack`, `alert_ack`, `clear_unread`, `list`, `audit_push`, `auth_logout`, `auth_session_status`, `get`, `set`, `user_presence_ping`.
...
Campos/dados de entrada identificados no HTML: `editNomeCompleto` (text), `editUsername` (text), `editDepartamentoPrincipal` (select), `editRamal` (text), `editEmailCorporativo` (email), `editSenha` (password), `editSenhaConfirmacao` (password), `transferTarget` (select), `addSemRespSelect` (select), `checkAllClientes` (checkbox), `perfilRole` (select), `perfilDepts` (select)
...
- **Finalidade:** endpoint identificado no JavaScript, mas sem action literal suficiente para detalhamento automático.
- **Dados enviados:** ponto a validar no código/backend.
- **Dados retornados:** JSON ou arquivo conforme endpoint.
- **Validações esperadas:** autenticação, autorização, CSRF para mutações, validação de entrada e tratamento de erro sem detalhes internos.
- **Riscos de segurança:** endpoint genérico pode concentrar múltiplas responsabilidades e dificultar auditoria.
- **Melhorias recomendadas:** separar actions por controller/service, documentar OpenAPI e criar testes de permissão.
...
Dados sensíveis envolvidos: dados de colaborador, credenciais, role, permissões, clientes vinculados e status. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
- Arquivos JavaScript extensos/acoplados à tela: `DEV/dev-data.js` (2582 linhas), `DEV/dev-colaborador.js` (1194 linhas), `global.js` (2433 linhas).
...
A página `DEV/dev-colaborador.html` atua no módulo **DEV / Colaboradores** e manipula **dados de colaborador, credenciais, role, permissões, clientes vinculados e status**. Os principais riscos são: administração privilegiada. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
...
- **Arquivo HTML atual:** `DEV/dev-colaborador.html`
- **Rota React implementada:** `/dev/colaboradores`
- **Componente React implementado:** `DevCollaboratorsPage.tsx`
...
- `CollaboratorsModule` foi criado com `GET/POST /api-v2/collaborators` e `GET/PATCH/DELETE /api-v2/collaborators/:id`.
- Mutacoes exigem permissoes `collaborators.create/update/delete`, CSRF e auditoria.
- O modelo `User` agora possui campos funcionais de colaborador (`position`, `phone`, `extension`, `metadata`, `archivedAt`, `archivedById`).
- Ainda faltam carteira/vinculos, backfill legado e integracao da tela DEV/React.

## 17. Atualizacao de frontend React - 2026-05-19 17:56

- A rota `/dev/colaboradores` passou a renderizar `DevCollaboratorsPage.tsx`.
- A tela consome `GET /api-v2/collaborators` para filtros/listagem e `PATCH/DELETE /api-v2/collaborators/:id` para edicao administrativa e arquivamento logico.
- Quando a sessao possui `roles.read`, consulta `GET /api-v2/roles` para opcoes de perfis; sem essa permissao, usa perfis operacionais conhecidos e mantem a validacao definitiva no backend.
- A area `/dev` ganhou atalho `Gerenciar colaboradores` condicionado a `collaborators.read`.
- A primeira fatia cobre busca, filtros, estatisticas, resumo seguro, status, senha opcional, perfis, dados funcionais e arquivamento por permissao. Transferencias/carteira continuam em `/manager/transferencias` e `/manager/colaboradores`.
