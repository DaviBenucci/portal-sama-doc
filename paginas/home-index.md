...
- **Perfil de usuário provável:** Qualquer usuário que acesse a rota legada `/HOME/index.html`
...
Redirecionar automaticamente para `/HOME/Inicio.html` por meta refresh.

- **Problema resolvido:** centraliza o fluxo de home para reduzir consulta manual, duplicação operacional e perda de rastreabilidade.
- **Quem utiliza:** Qualquer usuário que acesse a rota legada `/HOME/index.html`.
- **Etapa do fluxo:** Home dentro do Portal Sama.
- **Dados/documentos manipulados:** não manipula dados de negócio diretamente.
...
Dados sensíveis envolvidos: não manipula dados de negócio diretamente. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
A página `HOME/index.html` atua no módulo **Home** e manipula **não manipula dados de negócio diretamente**. Os principais riscos são: autorização por perfil/departamento e exposição de dados. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
...
- **Arquivo HTML atual:** `HOME/index.html`
