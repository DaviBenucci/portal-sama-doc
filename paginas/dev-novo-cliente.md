...
- **Arquivo HTML:** `DEV/dev-novo-cliente.html`
- **Módulo:** DEV / Clientes
- **Arquivos CSS relacionados:** `DEV/dev.css`, `global.css`, `https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css`
- **Arquivos JavaScript relacionados:** `auth.js`, `DEV/dev-data.js`, `DEV/dev-novo-cliente.js`, `global.js`
...
**Status backend 2026-05-13 10:50:** `ClientsModule` criou `POST /api-v2/clients` e `PATCH /api-v2/clients/:id` com aliases para campos desta tela (`cliRazaoSocial`, `cliCnpj`, `regime_tributario`, `id_empresa`, grupo), CSRF e auditoria. Falta conectar o frontend e executar backfill de clientes legados.

---

## 2. Objetivo da página

Cadastrar nova empresa cliente com dados cadastrais, contato, endereço, regime e grupo.

- **Problema resolvido:** centraliza o fluxo de dev / clientes para reduzir consulta manual, duplicação operacional e perda de rastreabilidade.
- **Quem utiliza:** Master ou administrador técnico.
- **Etapa do fluxo:** DEV / Clientes dentro do Portal Sama.
- **Dados/documentos manipulados:** nome, razão social, CNPJ, regime tributário, contato, endereço, status e grupo.
...
Conteúdo principal identificado: Portal Sama Contabilidade Cadastro de Novo Cliente Nome do colaborador ▾ Departamento do usuário Configurações do perfil Logout ← Voltar para Área DEV Novo clie; ← Voltar para Área DEV Novo cliente Cadastre uma nova empresa e preencha os dados principais. Dados da empresa Use informações oficiais para garantir consistênc; Dados da empresa Use informações oficiais para garantir consistência no cadastro e nos relatórios. Nome fantasia Razão social Classificação e identificação Esse.

Títulos/seções visíveis: `Portal Sama Contabilidade`, `Novo cliente`, `Dados da empresa`, `Classificação e identificação`, `Contato`, `Endereço`, `Grupo empresarial`.

Formulários identificados: `formNovoCliente` método `GET` com campos `cliNomeFantasia`, `cliRazaoSocial`, `cliCnpj`, `cliStatus`, `cliRank`, `cliRegimeTributario`, `cliIdEmpresa`, `cliAtividade`, `cliTelefone`, `cliEmail`, `cliEndereco`, `cliComplemento`.

Campos relevantes: `cliNomeFantasia` (text), `cliRazaoSocial` (text), `cliCnpj` (text), `cliStatus` (select), `cliRank` (select), `cliRegimeTributario` (select), `cliIdEmpresa` (text), `cliAtividade` (text), `cliTelefone` (text), `cliEmail` (email), `cliEndereco` (text), `cliComplemento` (text), `cliCep` (text), `cliBairro` (text), `cliCidade` (text), `cliEstado` (text), `cliFazParteGrupo` (select), `cliGrupoNome` (text).

Ações/botões identificados: `Início`, `Departamento ▾`, `Modelo padrão Departamento`, `Clientes`, `Área Dev`, `Nome do colaborador ▾`, `Departamento do usuário`, `Configurações do perfil`, `Logout`, `← Voltar para Área DEV`, `Salvar cliente`, `Cancelar`.
...
Campos/dados de entrada identificados no HTML: `cliNomeFantasia` (text), `cliRazaoSocial` (text), `cliCnpj` (text), `cliStatus` (select), `cliRank` (select), `cliRegimeTributario` (select), `cliIdEmpresa` (text), `cliAtividade` (text), `cliTelefone` (text), `cliEmail` (email), `cliEndereco` (text), `cliComplemento` (text), `cliCep` (text), `cliBairro` (text), `cliCidade` (text), `cliEstado` (text), `cliFazParteGrupo` (select), `cliGrupoNome` (text)
...
Dados sensíveis envolvidos: nome, razão social, CNPJ, regime tributário, contato, endereço, status e grupo. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
A página `DEV/dev-novo-cliente.html` atua no módulo **DEV / Clientes** e manipula **nome, razão social, CNPJ, regime tributário, contato, endereço, status e grupo**. Os principais riscos são: administração privilegiada. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
...
- **Arquivo HTML atual:** `DEV/dev-novo-cliente.html`
- **Rota React sugerida:** `/dev/clientes/novo`
- **Componente React sugerido:** `DevCreateClientPage.tsx`
...
## 16. Atualizacao de frontend React - 2026-05-18 15:07

- A rota `/dev/clientes/novo` passou a renderizar `DevNewClientPage.tsx`.
- A tela consome `POST /api-v2/clients` por `clients.service.ts`, usando `clientFormSchema`, React Hook Form e CSRF emitido pelo service antes da mutacao.
- A interface cobre razao social, nome fantasia, CNPJ, status, rank, ID da empresa, regime, atividade, contato, endereco, UF e grupo empresarial.
- A area `/dev` ganhou atalho condicionado a `clients.create`; apos cadastro, a tela mostra feedback e link para `/clientes/:id/painel`.
- Permanecem pendentes validacao com API v2/MySQL/usuarios reais, backfill de clientes legados, vinculos cliente-usuario/gestor/carteira, Playwright e homologacao HTTPS.
