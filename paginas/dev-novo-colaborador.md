...
- **Arquivo HTML:** `DEV/dev-novo-colaborador.html`
...
- **Arquivos JavaScript relacionados:** `auth.js`, `DEV/dev-data.js`, `DEV/dev-novo-colaborador.js`, `global.js`
...
Cadastrar colaborador, credenciais iniciais, role, departamento e permissões de clientes/departamentos.
...
- **Dados/documentos manipulados:** dados pessoais, login, senha provisória, cargo, departamento, role e permissões.
...
Conteúdo principal identificado: Portal Sama Contabilidade Cadastro de Novo Colaborador Nome do colaborador ▾ Departamento do usuário Logout ← Voltar para Área DEV Novo colaborador Cadastre um; ← Voltar para Área DEV Novo colaborador Cadastre um colaborador, defina o perfil de acesso e as permissões por departamento. Identificação Def; Identificação Defina os dados pessoais básicos e o identificador de login do colaborador. Nome Sobrenome Usuário (login) Use um padrão único e simples para faci.

Títulos/seções visíveis: `Portal Sama Contabilidade`, `Novo colaborador`, `Identificação`, `Perfil de acesso`, `Credenciais iniciais`, `Permissões e carteira de clientes`.

Formulários identificados: `formNovoColab` método `GET` com campos `colabNome`, `colabSobrenome`, `colabUsername`, `colabRamal`, `colabEmailCorp`, `colabDepartamento`, `colabCargo`, `colabStatus`, `colabSenha`, `colabSenhaConfirma`, `colabPermDept`, `colabClientes`.

Campos relevantes: `colabNome` (text), `colabSobrenome` (text), `colabUsername` (text), `colabRamal` (text), `colabEmailCorp` (email), `colabDepartamento` (select), `colabCargo` (select), `colabStatus` (select), `colabSenha` (password), `colabSenhaConfirma` (password), `colabPermDept` (select), `colabClientes` (select).

Ações/botões identificados: `Início`, `Departamento ▾`, `Modelo padrão Departamento`, `Clientes`, `Área Dev`, `Nome do colaborador ▾`, `Departamento do usuário`, `Logout`, `← Voltar para Área DEV`, `Salvar colaborador`, `Cancelar`.
...
- Uso de armazenamento no navegador identificado nos scripts relacionados: `localStorage` em 43 ocorrência(s) e `sessionStorage` em 8 ocorrência(s). Tokens/segredos não devem ser armazenados nesses mecanismos; manter apenas estado não sensível.
...
Campos/dados de entrada identificados no HTML: `colabNome` (text), `colabSobrenome` (text), `colabUsername` (text), `colabRamal` (text), `colabEmailCorp` (email), `colabDepartamento` (select), `colabCargo` (select), `colabStatus` (select), `colabSenha` (password), `colabSenhaConfirma` (password), `colabPermDept` (select), `colabClientes` (select)
...
Dados sensíveis envolvidos: dados pessoais, login, senha provisória, cargo, departamento, role e permissões. Não expor esses dados no HTML inicial, em logs do navegador, em mensagens de erro, em query strings permanentes ou em `localStorage`.
...
- Uso de `localStorage` em scripts relacionados (43 ocorrência(s)); não armazenar tokens ou dados sensíveis.
...
A página `DEV/dev-novo-colaborador.html` atua no módulo **DEV / Colaboradores** e manipula **dados pessoais, login, senha provisória, cargo, departamento, role e permissões**. Os principais riscos são: administração privilegiada. A principal recomendação é reforçar validações server-side, autorização por recurso, auditoria e componentização gradual.
...
- **Arquivo HTML atual:** `DEV/dev-novo-colaborador.html`
- **Rota React sugerida:** `/dev/colaboradores/novo`
- **Componente React sugerido:** `DevCreateCollaboratorPage.tsx`
...
- `POST /api-v2/collaborators` foi criado para cadastro inicial de colaborador.
- A rota exige JWT, `collaborators.create`, CSRF, senha hasheada com `bcrypt`, roles existentes e auditoria sem senha/hash.
- A API bloqueia role `CLIENT` no fluxo de colaborador e preserva aliases de entrada (`usuario`, `nome`, `departamento`, `cargo`, `telefone`, `ramal`, `perfis`).
- Ainda faltam adaptar a tela DEV/futuro React, backfill de usuarios legados e validacao em homologacao.

---

## 17. Atualizacao de frontend React - 2026-05-18 15:14

- A rota `/dev/colaboradores/novo` passou a renderizar `DevNewCollaboratorPage.tsx`.
- A tela consome `POST /api-v2/collaborators` por `collaborators.service.ts`, usando `collaboratorCreateSchema`, React Hook Form e CSRF emitido pelo service antes da mutacao.
- Quando a sessao possui `roles.read`, consulta `GET /api-v2/roles`; sem essa permissao, usa perfis operacionais conhecidos e mantem a validacao definitiva no backend.
- A area `/dev` ganhou atalho condicionado a `collaborators.create`; apos cadastro, mostra feedback e link para `/clientes/colaboradores/:id`.
- Permanecem pendentes validacao com API v2/MySQL/usuarios reais, backfill de colaboradores/usuarios legados, vinculos colaborador-cliente/gestor, carteira/transferencias, Playwright e homologacao HTTPS.
