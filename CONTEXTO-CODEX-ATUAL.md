...
- Backend de Faturamento implementado no `portal-sama-api`: controller autenticado `/api-v2/accounting/faturamento`, service adaptador para `portal-sama-api/services/faturamento/script/processar_faturamento.py`, DTOs de entrada e envs `SAMA_FATURAMENTO_*`.
- Correcao pos-producao da 12.1.1: pacote Python migrado para `portal-sama-api/services/faturamento`; fallback para `Downloads\Faturamento` removido; `Dockerfile` instala `services/faturamento/requirements.txt`; o fluxo usa `ACESSORIAS_TOKEN` do `.env` principal da API, sem `.env` separado no pacote.
- Adaptacao final para o Portal: runtime do Faturamento fica em `STORAGE_PRIVATE_PATH/faturamento/jobs/<jobId>` por padrao, com override `SAMA_FATURAMENTO_WORK_DIR`; `services/faturamento` guarda apenas script/modelo/documentacao. Scripts legados foram analisados e nao promovidos por terem caminhos/parametros fixos e problemas de robustez.
- Ajuste operacional seguinte: o Python remove os PDFs temporarios baixados do Acessorias depois de gerar CSV/XLSX; o frontend exibe botoes autenticados para baixar `CSV Dominio` e `Livro Caixa`; falhas sem PDF agora retornam os ultimos motivos do Acessorias por competencia.
- O adaptador executa o Python somente no backend, injeta `ACESSORIAS_TOKEN` e `API_TOKEN` por ambiente, aceita `codigo_empresa` obrigatorio do Acessorias e `codigo_dominio` opcional do Dominio, retorna status/meses/artefatos e fornece downloads autenticados de CSV, planilha e log.
