...
Status em 2026-05-19 12:16: a primeira fatia de transferencias foi modelada no Prisma com `TransferSession`, usando a tabela legada `sama_transfer_sessions` e migration `20260519120000_add_transfer_sessions` com criacao/indices condicionais para reduzir risco em bancos onde a tabela ja exista. Em 2026-06-02, a leitura do dashboard e a escrita das transferencias em lote passaram a usar/alimentar `client_department_assignments` quando ha departamento controlado aplicado. `Client.metadata.departamentos/depts/dept_*` e `User.metadata.clientes/colaboradorId` continuam como fallback temporario ate backfill e validacao real.

Status em 2026-06-02: a responsabilidade formal de clientes avancou localmente com `Department` e `ClientDepartmentAssignment`, usando as migrations `20260527153000_add_controlled_departments` e `20260527162000_add_client_department_assignments`. A UI React `/clientes/:id` ja le, cria, edita, encerra e transfere atribuicoes pela tabela `client_department_assignments`, incluindo gestor opcional. O `TransfersModule` tambem passou a escrever localmente nessa tabela ao aplicar/retornar transferencias em lote. O comando `npm run ops:client-assignments:backfill` foi criado para popular responsabilidades a partir de `clients.metadata`, com dry-run por padrao e `--apply` explicito; `ops:backfill:report` tambem conta a tabela normalizada e duplicidades `PRIMARY ACTIVE`. Complemento em 2026-06-02 16:00: `CalendarModule`, `DepartmentsModule` e aplicacao Acessorias no workspace ja carregam/priorizam responsabilidades ativas dessa tabela em vencimentos e planilhas departamentais, mantendo `clients.metadata` como fallback quando nao ha dado normalizado. Complemento em 2026-06-02 16:30: `DocumentsModule` passou a usar a mesma tabela para escopo local de documentos internos, com fallback por `document.department` quando nao ha atribuicao ativa normalizada. A validacao real ainda depende de aplicar migrations/seeds no MySQL real, executar/conferir o backfill no EasyPanel e validar com dados reais.
...
Status complementar em 2026-06-01: a mesma modelagem passou a ser usada para aplicacao multidepartamental. O campo `department` diferencia Fiscal, Contabil, Pessoal, Financeiro e Legalizacao, enquanto as celulas ficam em metadados separados por departamento (`fiscalSheet`, `contabilSheet`, `pessoalSheet`, `financeiroSheet`, `legalizacaoSheet`) ate existir tabela relacional propria de planilha. Em 2026-06-02 16:00, a aplicacao passou a consultar `client_department_assignments` antes de escrever nas planilhas e registra `CLIENT_DEPARTMENT_MISMATCH` quando a responsabilidade normalizada aponta para outro departamento.
...
Status em 2026-05-20 16:02: a exclusao de vencimentos do `CalendarModule` opera sobre os modelos ja mapeados `CalendarRule`, `CalendarRuleCompany` e `CalendarDueNotified`, removendo o vinculo empresa-regra, limpando notificacoes por regra/empresa/departamento e excluindo a regra quando ela fica sem vinculos. Em 2026-06-02, configuracao, listagem mensal e exclusao de vencimentos passaram a carregar `client_department_assignments` ativas dos clientes e a priorizar departamentos normalizados antes do fallback por `Client.metadata`. Nao houve nova migration nesta fatia; validacao com MySQL/homologacao segue pendente.
...
Status em 2026-06-02 16:30: o escopo interno de documentos passou a consultar `client_department_assignments` ativas para usuarios departamentais. Quando ha responsabilidade normalizada para o cliente, ela precisa coincidir com o departamento do documento e do usuario; quando nao ha responsabilidade normalizada, o fallback temporario por `document.department` permanece ate backfill/conferencia real.
